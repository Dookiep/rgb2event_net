...
class EventFrameHead(nn.Module):
    def __init__(self, in_channels, out_channels=2, time_steps=6):
        super(EventFrameHead, self).__init__()
        
        # Reduction and transformation layers
        self.reduction = nn.Sequential(
            nn.Conv2d(in_channels, in_channels // 2, kernel_size=3, padding=1),
            nn.BatchNorm2d(in_channels // 2),
            nn.PReLU(),
            nn.Conv2d(in_channels // 2, out_channels * time_steps, kernel_size=1)
        )
    ...
    def forward(self, x):
        # x is the final feature map from the last stage
        # Shape: [B, C, H, W]
        event_frames = self.reduction(x)
        
        # Reshape to [B, time_steps, out_channels, H, W]
        event_frames = event_frames.view(
            x.size(0), 
            self.time_steps, 
            self.out_channels, 
            event_frames.size(2), 
        ...
        # Permute to [B, time_steps, out_channels, H, W]
        event_frames = event_frames.permute(0, 1, 2, 3, 4)
...
class MotionFormer(nn.Module):
    def __init__(self, in_chans=3, embed_dims=[32, 64, 128, 256, 512], motion_dims=64, num_heads=[8, 16], 
        ...
        cur = 0
        self.event_head = EventFrameHead(
            in_channels=embed_dims[-1],  # Use the channels of the last stage
            out_channels=2,  # Positive and negative events
            time_steps=6
        )
    ...
    def forward(self, x1, x2):
        ...
        for i in range(self.num_stages):
            ...
            appearence_features.append(x)
              # Use the last stage appearance features to generate event frames
            last_stage_features = appearence_features[-1]
        
            # Generate event frames
            event_frames = self.event_head(last_stage_features)
...
def event_mse_loss(pred_events, gt_events):
    """
    Compute MSE loss between predicted and ground truth event frames
    
    Args:
    - pred_events: Predicted event frames [B, T, C, H, W]
    - gt_events: Ground truth event frames [B, T, C, H, W]
    
    Returns:
    - MSE loss
    """
    return nn.functional.mse_loss(pred_events, gt_events)

def feature_extractor(**kargs):
    model = MotionFormer(**kargs)
    return model

def test_motionformer_forward():
    # Create a model with default parameters
    model = MotionFormer(
        in_chans=3,  # RGB input
        embed_dims=[32, 64, 128, 256, 512],  # example embedding dimensions
        motion_dims=[64, 64, 64, 64],  # motion dimensions
        num_heads=[8, 16],  # number of attention heads
        depths=[2, 2, 2, 6, 2],  # depth of each stage
        window_sizes=[11, 11]  # window sizes
    )

    # Create random input tensors
    # Batch size of 4, 2 consecutive frames
    batch_size = 4
    
    # Create two random RGB video frames
    x1 = torch.randn(batch_size, 3, 224, 224)  # First frame
    x2 = torch.randn(batch_size, 3, 224, 224)  # Second frame

    # Perform forward pass
    try:
        event_frames = model(x1, x2)
        ...
        print("Expected shape: [Batch, Time_steps, Channels, Height, Width]")
        print(f"Expected: [4, 6, 2, {event_frames.shape[3]}, {event_frames.shape[4]}]")
        
        # Additional shape validation
        assert event_frames.ndim == 5, "Output should be 5-dimensional"
        assert event_frames.shape[0] == batch_size, "Batch size mismatch"
        assert event_frames.shape[1] == 6, "Time steps should be 6"
        assert event_frames.shape[2] == 2, "Should have 2 event channels"
